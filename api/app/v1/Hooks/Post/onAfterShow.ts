...
export default async ({ req, item }: IAfterShowContext) => {
  ...
  let myLikedPostIds: any[] = [];
  let mySharedPostIds: any[] = [];

  if (userId) {
    // The posts that I liked
    myLikedPostIds = await PostService.getMyLikedPostIds(userId, postIds);

    // The post ids that I already shared
    mySharedPostIds = await PostService.getMySharedPostIds(userId, postIds);
  }
